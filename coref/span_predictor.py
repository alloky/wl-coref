...
class SpanPredictor(torch.nn.Module):
    ...
    def forward(self,  # type: ignore  # pylint: disable=arguments-differ  #35566 in pytorch
        ...
        # print(heads_ids.shape)
        # print(heads_ids)

        window_heads_ids = heads_ids
        # print(window_heads_ids)
        left = (window_heads_ids >= window_start).nonzero(as_tuple=True)[0][0]
        # print((window_heads_ids >= window_start).nonzero(as_tuple=True)[0][:10])
        right = (window_heads_ids < window_end).nonzero(as_tuple=True)[0][-1] + 1
        # print((window_heads_ids >= window_start).nonzero(as_tuple=True)[0][-10:])
        window_heads_ids = window_heads_ids[left:right]
        # print(window_heads_ids)

        # Obtain distance embedding indices, [n_heads, n_words]
        relative_positions = (window_heads_ids.unsqueeze(1) - torch.arange(window_size, device=words.device).unsqueeze(0))
        ...
        sent_id = torch.tensor(doc["sent_id"][window_start:window_end], device=words.device)

        same_sent = (sent_id[window_heads_ids - window_start].unsqueeze(1) == sent_id.unsqueeze(0))
        ...
        scores = torch.full(
            (window_heads_ids.shape[0], words[window_start:window_end].shape[0], 2),
            float('-inf'),
            device=words.device
        )
    ...
    def get_training_data(self,
        ...
        return self(doc, words, heads, window_start, window_end), (starts, ends)
    ...
    def predict(self,
        ...
        scores = self(doc, words, heads_ids, window_start, window_end)
        starts = scores[:, :, 0].argmax(dim=1).tolist()
        ends = (scores[:, :, 1].argmax(dim=1) + 1).tolist()

        # print("HERE")
        # print(heads_ids.shape, len(starts), len(ends))
        # print(heads_ids)

        left = (heads_ids >= window_start).nonzero(as_tuple=True)[0][0]
        right = (heads_ids < window_end).nonzero(as_tuple=True)[0][-1] + 1
        window_heads_ids = heads_ids[left:right]

        head2span = {
            head: (start, end)
            for head, start, end in zip(window_heads_ids.tolist(), starts, ends)
        }

        # print(clusters)
        # print(head2span)

        return [[head2span[head] for head in cluster if head in head2span]
